...
layer_dims = calculate_diamond_dims(config['n_layer'], config['base_dim'], config['max_dim'], config['head_dim'])
...
model_args = dict(
    ...
    n_layer=config['n_layer'], # Add new layer dimension
...
if config.get('wandb_log', False) and master_process:
    ...
    wandb.watch(model, log="all") # Optional: Watch the model for gradients and parameters
